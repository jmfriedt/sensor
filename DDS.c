...
// unsigned int parab[NBPICS_MAX * ANTENNES_MAX];
  ...
  unsigned short safran_dac1=0,safran_dac2=0;
#ifdef dephaseur
...
//              eMBRTUSend(0x01,&cmd_modbus[1],5);   // 5 car elimine le 1er et les 2 derniers du CRC
  ...
  init_sliding_avg();
#endif
/* DAC SAFRAN : AD5752 sur port d'extension (16-bit DAC) */
 if (safran_temperature==1)
   {safran_init();                             
    safran_dac(0,32765); safran_dac(2,32765);  
   }
/*  
  while (1) {safran_dac(0,safran_dac1);safran_dac1++;  // voir ligne 3591
             safran_dac(2,safran_dac2);safran_dac2+=2;
             if (safran_dac1>=65530) safran_dac1=0;
             if (safran_dac2>=65530) safran_dac2=0;
...
//#endif
...
#endif
		      if (((convertit_temperature == 1)||(safran_temperature == 1)) &&((npic%2)==0))
			  ...
			  && (variance[npic+1+antenne*NBPICS_MAX]<SEUILVAR))
			  {
			   if (nbpics[antenne]>1) {
#ifndef AD9958
		  	    tempracine=A2[(antenne*NBPICS_MAX+npic)/2]*(((((((parab_conti[npic+1+antenne*NBPICS_MAX]-parab_conti[npic+0+antenne*NBPICS_MAX])>>4)*40)/859)*16)/10)/100);
		  	    tempracine=A1[(antenne*NBPICS_MAX+npic)/2]+(tempracine); // /100);	// 191206 : depassement capacite' Safran
#else
  		  ...
  		  //tempracine=A1[npic/2]+A2[npic/2]*((((((parab_conti[npic+1+antenne*NBPICS_MAX]-   // remplacer par 1 et 0 si 1er pic pour T
			  ...
			  //  parab_conti[2+antenne*NBPICS_MAX])>>4)*40)/859)*16)/10); // 06/02/2012
			    if (tempracine < 0) {tempracine = 0; jmf_putchar ('*',NULL,NULL,usart_port);}
...
// TEST 191130
//if (npic==0) tempracine=600;
//if (npic==2) tempracine=750;
#ifndef dephaseur
                           if (npic==0)  // map 0..1000 a 0..65536
                              safran_dac(0,(tempracine*8192)/125); // 2^16/1000=2^13/125 et T\in[0..1000] sur 10 bits
                           else 
                              safran_dac(2,(tempracine*8192)/125);
